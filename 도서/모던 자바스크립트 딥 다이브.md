...
배열 요소중 최댓값 구하려면 apply 메서드 쓰거나 스프레드 문법 사용해야 한다.
...
new 없이 호출하면 시간 정보를 문자열로 반환
Date() // "Mon Jul 06 2020 01:10:47 GMT+0900(대한민국 표준시)"

new Date('2020/03/26/10:00:00');
=> Thu Mar 26 2020 10:00:00 GMT+0900(대한민국 표준시)

new Date(2020, 3, 26, 10, 00, 00, 0)
=> Thu Mar 26 2020 10:00:00 GMT+0900(대한민국 표준시)
...
test 메서드는 target 문자열에 대해 정규 표현식 regexp의 패턴 검색하여 매칭 결과를 불리언 값으로 반환한다.
regexp.test(target) // true
```

```js
RegExp.exec
인수로 전달받은 문자열에 대해 정규 표현식의 패턴 검색하여 매칭 결과를 배열로 반환한다. 결과 없는 경우 null 반환
...
RegExp.test
인수로 전달받은 문자열에 대해 정규 표현식 패턴 검색하여 매칭 결과를 불리언 값으로 반환
...
target.match(regExp);
=> ["is", index: 5, input: 'Is this all there is?', groups: indefined]
...
=> 플래그는 옵션이므로 선택적 이다. 순서와 상관없이 하나 이상의 플래그를 동시에 설정 가능
패턴 검색 매칭 대상이 1개 이상 존재해도 첫 번째 매칭한 대상만 검색하고 종료한다.

#### 패턴

- 일정한 규칙 가진 문자열의 집합 표현하기 위해 사용하는 형식 언어
...
const regExp = /A{2}/g; => {2} 는 {2, 2} 와 같다. 2번 반복되는 문자 찾기
target.match(regExp)
=> ["AA", "AA"]

const regExp = /A{2, }/g => {2, } 2번 이상 반복되는 문자 찾기
target.match(regExp)
=> ["AA", "AAA"]

const regExp = /A+/g => +는 {1, } 와 같음 1번 이상 반복되는 문자 찾기
target.match(regExp)
=> ["A", "AA", "A", "AAA"]

const regExp = /colou?r/g => colo 다음 u가 0번 이상 반복되고 r이 이어지는 문자열 찾기
=> ["color", "colour"]
```
...
/A+|B+/ => A 한번 이상, B 한번이상 나오는 문자열

[]내의 문자는 or로 동작한다.
/[AB]+ => A 한번 이상, B 한번이상 나오는 문자열
=> ["A", "AA", "B", "BB", "A", "B"]

범위는 [] 내에 - 사용
/[A-Z]+/

대소문자 구별하지 않고 알파벳 검색 방법
/[A-Za-z]+/
=> ["AA", "BB", "Aa", "Bb"]

/[0-9]+/ => "0" ~ "9" 가 한 번 이상 반복되는 문자열 전역 검색

const target = "AA BB 12,345"

/[\d,]+/ => '0' ~ '9' 또는 ','가 한 번 이상 반복되는 문자열 전역 검색
=> ['12,345']

/[\D,]+/ '0' ~ '9'가 아닌 문자(숫자가 아닌 문자) 또는 ','가 한 번 이상 반복되는 문자열 전역 검색
=> ["AA BB ", ","]

\w 는 알파벳, 숫자, 언더스코어 의미 \w === [A-Za-z0-9_]와 같음 \W는 \w와 반대로 동작한다.
```
...
```js
/^[A-Za-z0-9]{4, 10}$/ => 알파벳 대소문자 또는 숫자로 시작하거나 끝나며 4 ~ 10 자리인지 검사
...
strObj;
// String {0: "L", 1: "e"} String 래퍼 객체 생성
```

문자열은 원시 값이므로 변경할 수 없다

```js
new 연산자 사용하지 않고 String 생성자 함수를 호출하면
...
```js
String.search
인수로 전달받은 정규 표현식과 매치하는 문자열 검색하여 일치하는 문자열의 인덱스 반환, 실패하면 -1 반환
...
String.includes
인수로 전달받은 문자열 포함되어 있는지 true, false 반환

str.includes('Hello') // true
str.includes('l', 3) // true 두 번째 인자로 검색 시작할 인덱스 전달

String.startsWith
인수로 전달받은 문자열로 시작하는지 확인하여 true, false 반환

str.startsWith('He') // true
str.startsWith(' ', 5) // true 두 번째 인자로 검색 시작할 인덱스 전달

String.endsWith
전달받은 문자열로 끝나는지 확인하여 true, false 반환
...
String.slice => 파이썬 슬라이싱 이랑 같음

String.toUpperCase => 대문자 반환
String.toLowerCase => 소문자 반환

String.trim => 대상 문자열 앞뒤 공백 문자 있을 경우 공백 제거한 문자열 반환
String.trimStart() => 앞 공백 제거 한 문자열 반환
String.trimEnd() => 뒤 공백 제거 한 문자열 반환

str.replace(/\s/g, '') => 공백 제거
...
str.repeat(-1) // RngeError

String.replace
첫 번째 인수로 받은 문자열 또는 정규표현식을 검색하여 두 번째 인수로 받은 문자열로 치환한 문자열 반환
...
객체가 생성되는것이 아니라 변경 불가능한 원시 값이 생성되기 때문
...
심벌 값에 대한 설명이 같더라도 유일무이한 심벌 값 생성한다.
...
mySymbol.toString(); // Symbol(mySYmbol)
...
Symbol.for 메서드는 심벌 레지스트리에서 인수로 전달받은 문자열을 키로 사용하는 심벌 값 검색한다.

- 검색 성공하면 새로운 심벌 값 생성하지 않고 검색된 심벌 값 반환한다.
- 검색 실패하면 새롤운 심벌 값 생성하여 인수로 전달된 키로 전역 전역 심벌 레지스트리에 저장한 후 심벌 값 반환한다.
...
Symbol.for 메서드 사용하면 애플리케이션 전역에서 중복되지 않는 심벌 값 단 하나만 생성하여 전역 레지스트리를 통해 공유할 수 있다.



Symbol.keyFor 메서드는 전역 심벌 레지스트리에 저장된 심벌 값의 키 추출할 수 있다.
const s1 = Symbol.for('mySymbol')
...
심벌 값을 프로퍼티 키로 사용하려면 대괄호 사용해야한다.
const obj = {
  [Symbol.for('mySymbol')]: 1
}
심벌 값을 키로 사용하는 경우 for ... in 문이나 Object.getOwnPropertyNames 메서드로 찾을 수 없다.
...
프로그래밍 언어를 실행하기 위해 문자열을 토큰으로 분해하고 문법적 의미와 구조를 반영하여 트리 구조의 자료구조인
파스 트리를 생성하는 일련의 과정
일반적으로 파싱이 완료된 후에는 파스 트리 기반으로 중간 언어인 바이트코드 생성하고 실행한다.
...
1. 브라우저는 HTML, CSS, 자바스크립트, 이미지, 폰트 파일 등 렌더링에 필요한 리소스 요청하고 서버로부터 응답 받음
2. 브라우저 렌더링 엔진은 서버로부터 응답된 HTML과 CSS 파싱하여 DOM, CSSOM 생성하고 이들을 결합하여 렌더 트리 생성
3. 브라우저는 서버로부터 받은 자바스크립트를 파싱하여 AST 생성, 바이트코드로 변환하여 실행한다.
   이때 자바스크립트는 DOM API 통해 DOM, CSSOM을 변경할 수 있다. 변경된 DOM, CSSOM은 다시 렌더 트리로 결합된다.
4. 렌더 트리 기반으로 HTML 요소의 레이아웃 계산하고 화면에 페인팅한다.

#### 요청과 응답

브라우저의 렌더링 엔진이 HTML을 파싱하는 도중 외부 리소스 로드하는 태그(CSS link 태그, img 태그, 자바스크립트 script 태그)를
만나면 HTML 파싱 중단하고 해당 리소스 파일을 서버로 요청하기 때문

#### HTML 파싱과 DOM 생성

HTML 문서를 렌더링하려면 HTML 문서를 브라우저가 이해할 수 있는 자료구조(객체)로 변환하여 메모리에 저장해야 한다.

1. 브라우저의 요청한 HTML 파일을 응답하는데 이때 서버는 HTML 파일 읽어 메모리에 저장하고 메모리에 저장된 바이트(2진수)를 인터넷을 경유하여 응답한다.
2. - 브라우저는 서버가 응답한 HTML문서를 바이트 형태로 응답받는다.
   - 응답된 바이트 형태의 HTML 문서는 meta 태그의 charset 어트리뷰트에 의해 지정된 인코딩 방식(ex UTF-8)을 기준으로 문자열로 변환
3. 문자열로 변환된 HTML 문서 읽어들여 문법적 의미 갖는 코드의 최소 단위인 토큰들로 분해한다.
4. - 토큰들을 객체로 변환하여 노드 생성
   - 토큰 내용에 따라 문서 노드, 요소 노드, 어트리뷰트 노드, 텍스트 노드가 생성 된다.
   - 노드는 이후 DOM 구성하는 기본 요소가 된다.

5. - HTML 문서는 HTML 요소들의 집합으로 이뤄지며 요소는 중첩 관계를 갖는다.
   - HTML 요소의 콘텐츠 영역(시작 태그 ~ 종료 태그 사이)에는 텍스트 뿐 아니라 다른 HTML 요소도 포함될 수 있다.
   - HTML 요소 간에는 중첩 관계에 의해 부자 관계 형성된다.
   - 부자 관계 반영하여 모든 노드들을 트리 자료구조로 구성 => DOM이라 한다.

#### CSS 파싱과 CSSOM 생성

1. 렌더링 엔진은 DOM 생성해 나가다가 CSS 로드하는 link 태그나 style 태그 만나면 DOM 생성 일시 중단한다.
2. link 태그의 href 어트리뷰트에 지정된 CSS 파일을 서버에 요청한다.
3. CSS 파일이나 style 태그 내의 CSS를 바이트 -> 문자 -> 노드 -> CSSOM 거치며 해석하여 CSSOM 생성한다.
4. CSS 파싱 완료하면 HTML 파싱 중단된 지점부터 다시 HTML 파싱하기 시작하여 DOM 생성 재개한다.

#### 렌더 트리 생성

HTML, CSS 파싱하여 각각 DOM, CSSOM 생성한다.
DOM, CSSOM은 렌더링 위해 렌더 트리로 결합된다.

> 렌더 트리는 렌더링 위한 트리 자료구조 이므로 브라우저 화면에 렌더링 되지 않는 노드(ex meta, script 태그, css display:none) 되는
> 노드들은 포함하지 않는다.
> 즉 브라우저 화면에 렌더링되는 노드만으로 구성된다.

#### 자바스크립트 파싱과 실행

HTML 문서 파싱한 결과물로 생성된 DOM은 HTML 요소와 스타일 등을 변경할 수 있는 프로그래밍 인터페이스로서 DOM API 제공한다.

1.  HTML 한 줄씩 파싱하며 DOM 생성하다 자바스크립트 파일 로드하는 script 만나면 DOM 생성 일시 중단한다.
2.  script 태그의 src 속성에 정의된 파일 서버에 요청한다.
3.  자바스크립트 파일, script 태그 내의 자바스크립트 코드 파싱하기 위해 자바스크립트 엔진에 제어권 넘김
4.  자바스크립트 파싱 실행 종료되면 렌더링 엔진으로 다시 제어권 넘겨 DOM 생성 재개한다.
5.  자바스크립트 엔진은 js 코드 파싱하여 CPU가 이해할 수 있는 저수준 언어로 변환하고 실행하는 역할 한다.
6.  자바스크립트 엔진은 코드 해석하여 AST(추상적 구문 트리) 생성한다.
7.  AST 기반으로 인터프리터가 실행 할 수 있는 중간 코드인 바이트코드 생성하여 실행한다.

토크나이징(어휘 분석): 자바스크립트 소스코드 ------------> 토크나이저
...
단, 자바스크립트 파싱과 실행은 자바스크립트 파일 로드 완료된 직후 진행되며 이때 HTML 파싱 중단된다.

#### defer 속성

async 와 마찬가지로 HTML 파싱과 외부 자바스크립트 로드가 비동기적으로 동시에 진행된다.

단, 자바스크립트 파싱과 실행은 HTML 파싱이 완료된 직후, DOM 생성 완료된 직후 진행된다.
=> DOM 생성 완료된 이후 실행되어야 할 자바스크립트에 유용하다.

# 39장 DOM

DOM은 HTML 문서의 계층적 구조와 정보를 표현하며 제어할 수 있는 API, 프로퍼티와 메서드 제공하는 트리 자료구조이다.

#### 노드

- HTML 요소는 렌더링 엔진에 의해 파싱되어 DOM 구성하는 요소 노드 객체로 변환된다.
- HTML 요소 어트리뷰트는 어트리뷰트 노드로
...
- 어트리뷰트 노드는 HTML 요소의 오소 노드와 연결되어 있다
- 어트리뷰트 노드는 부모 노드와 연결되어 있지 않고 요소 노드에만 연결되어 있다.
- 어트리뷰트 노드는 부모 노드가 없으므로 요소 노드의 형제 노드는 아니다.
- 어트리뷰트 노드에 접근하려면 먼저 요소 노드에 접근해야 한다.

**HTML 요소에 id 어트리뷰트 부여하면 id 값과 동일한 이름의 전역 변수가 암묵적으로 선언되고 해당 노드 객체가 할당되는 부수 효과가 있다.**
...
<scrip>
  ...
  console.log(elems) // HTMLCollections(3) [li.red, li.red, li.red]

  for(let i = 0 ; i < elems.length; i++) {
    elems[i].className = 'blue';
  }

  console.log(elems) // HTMLCollections(1) [li.red]
</script>

1. 첫 번째 반복 i === 0
첫 번째 li 클래스가 blue 로 변경되어 elems에서 실시간으로 제거된다.
2. i === 1
i 1은 마지막 li 태그를 가리키므로 class를 blue 로 변경되고 제거된다.

그래서 살아있는 객체인 HTMLCollections를 for 문으로 순회하면서 노드 객체의 상태를 변경해야 할땐 주의해야 한다.
for 문을 역방향으로 순회하는 방법으로 회피할 수 있다.
HTMLCollections 를 사용하지 않고 배열의 고차함수 쓰는것이 바람직하다.
```

#### NodeList

> getElementsByTagName, getElementsByClassName 메서드는 HTMLCollections 객체를 반환하므로 사용을 지양하고
> querySelectorAll, querySelector 메서드를 사용하여 NodeList 객체를 반환하는 것이 좋다.
> NodeList 는 노드 객체의 상태 변경을 반영하지 않는 객체이기 때문

**childNodes 프로퍼티가 반환하는 NodeList 객체는 HTMLCollection 객체와 같이 실시간으로 노드 객체의 상태 변경 반영하는 live 객체로 동작한다.**

**안전하게 DOM 컬렉션 사용하려면 HTMLCollection이나 NodeList 객체를 배열로 변환하여 사용하는 것을 권장한다.**

#### 자식 노드 탐색

| 프로퍼티                            | 설명                                                                                             |
| ----------------------------------- | ------------------------------------------------------------------------------------------------ |
| Node.prototype.childNodes           | 자식 노드 모두 탐색하여 NodeList에 담아 반환, 노드뿐 아니라 텍스트 노드도 포함되어 있을 수 있다. |
| Element.prototype.children          | 자식 중 요소 노드만 모두 탐색하여 HTMLCollection에 담아 반환한다. 텍스트 노드는 포함되지 않는다. |
| Node.prototype.firstCHild           | 첫 번째 자식 노드 반환, firstChild 프로퍼티가 반환한 노드는 텍스트 노드이거나 요소 노드이다.     |
| Node.prototype.lastChild            | 마지막 자식 노드 반환, 반환 노드는 텍스트 노드이거나 요소 노드이다.                              |
| Element.prototype.firstElementChild | 첫 번쨰 자식 요소 노드 반환, 요소 노드만 반환                                                    |
| Element.prototype.lastElementChild  | 마지막 요소 노드 반환, 요소 노드만 반환                                                          |
...
요소 노드로 직접 모든 텍스트를 취득할 수 있다.(HTML 마크업은 무시된다.)
textContent 프로퍼티에 문자열 할당하면 요소 노드의 모든 자식 노드 제거되고
할당한 문자열이 텍스트로 추가된다.(마크업 은 무시된다.)
```

#### insertAdjacentHTML 메서드

**Element.prototype.insertAdjacentHTML(position, DOMString) => 기존 요소 제거하지 않으면서 위치 지정해 새로운 요소 삽입한다.**

첫 번째 인수로 전달할 수 있는 문자열: beforebegin, afterbegin, beforeend, afterend

> 기존 요소에는 영향을 주지 않고 새롭게 삽입될 요소만을 파싱하여 자식 요소로 추가한다.
> 기존의 자식 노드 제거하고 처음부터 새롭게 자시 노드 생성하여 자식 요소로 추가하는 innerHTML 프로퍼티보다 효율적이고 빠르다

#### 텍스트 노드 생성

Document.prototype.createTextNode(text) 메서드는 텍스트 노드 생성하여 반환한다.
...
요소 노드에 자식 노드 있는 경우 요소 노드의 textContent 프로퍼티에 문자열 할당하면
...
<body>
  ...
  <script>
    ...
    hasAttribute 로 해당 어트리뷰트 있는지 확인
...
**HTML 어트리뷰트의 역할은 HTML 요소의 초기 상태 지정하는 것. 즉 HTML 어트리뷰트 값은 HTML 요소의 초기 상태 의미하며 이는 변하지 않는다.**
...
- 요소 노드는 상태를 가지고 있다.
- 요소 노드는 사용자의 입력에 의해 변화하는 살아있는 것이다.

**요소 노드는 2개의 상태인 초기 상태와 최신 상태를 관리해야 한다. 요소 노드의 초기 상태는 어트리뷰트 노드가 관리하며, 요소 노드의 최신 상태는 DOM 프로퍼티가 관리한다.**
...
- Symbol.iterator를 프로퍼티 키로 사용한 메서드 직접 구현하거나
- 프로토타입 체인 통해 상속 받은 Symbol.iterator 메서드 호출 시
  => 이터레이터 프로토콜 준수한 이터레이터 반환한다.
  **이터러블 프로토콜을 준수한 객체를 이터러블이라 한다. 이터러블은 for ... of 문 으로 순회 가능하고 스프레드 문법과 배열 디스트럭처링 할당 대상으로 사용할 수 있다.**

##### 이터레이터 프로토콜

- 이터러블의 Symbol.iterator 메서드 호출하면 이터레이터 프로토콜 준수한 이터레이터 반환한다.
- 이터레이터는 next 메서드 소유하고 next 메서드 호출하면 이터러블 순회하며 value 와 done 프로퍼티 갖는 이터레이터 리절트 객체를 반환한다.
- 이터레이터 프로토콜을 준수한 객체를 이터레이터라 한다.
- 이터레이터는 이터러블의 요소 탐색하기 위한 포인터 역할을 한다.

#### 이터러블

배열은 Array.prototype의 Symbol.iterator 메서드 상속받는 이터러블이다.
...
**이터러블의 Symbol.iterator 메서드가 반환한 이터레이터는 next 메서드 갖는다.**
...
'next' in iterator // true
```

- 이터레이터의 next 메서드는 이터러블의 각 요소 순회하기 위한 포인터 역할 한다.
- next 메서드 호출하면 이터러블을 순차적으로 한 단계씩 순회하며 순회 결과 나타내는 이터레이터 리절트 객체 반환한다.
...
next 메서드가 반환하는 이터레이터 리절트 객체의 value 프로퍼티는 현재 순회중인 이터러블의 값이고
...
**for (변수 선언문 of 이터러블) { ... }**
=> 이터레이터의 next 메서드 호출하여 이터러블 순회하며 next 메서드가 반환한 리절트 객체의 value 프로퍼티 값을 for ... of 문의 변수에 할당한다.

**for (변수 선언문 in 객체) { ... }**
=> 객체의 프로토타입 체인 상에 존재하는 모든 프로토타입 프로퍼티 중 프로퍼티 어트리뷰트 \[[Enumerable]] 값이 true인 프로퍼티 순회하며 열거한다.

```js
const iterable = [1, 2, 3];
const terator = iterable[Symbol.iterator]();
...
```js
Array.from 메서드는 유사 배열 객체 또는 이터러블 인수로 받아 배열로 변환하여 반환한다.
...
**Array, String, Map, Set, DOM 컬렉션(NodeList, HTMLCollection), arguments -> for ... of 문으로 순회할 수 있는 이터러블에 한정된다.**
...
이터러블이 아닌 일반 객체는 스프레드 문법 대상이 아니다.
...{a: 1, b: 2}
```

- 스프레드 문법의 결과는 값이 아님
- ...이 피 연산자를 연산하여 값 생성하는 연산자가 아님
...
**Rest 파라미터는 함수에 전달된 인수들을 배열로 전달 받기 위해 이름 앞에 ...을 붙이는 것**
...
프로퍼티가 중복되는 경우 뒤에 위치한 프로퍼티가 우선권 갖는다.
...
이터러블 또는 객체를 destructuring(비구조화, 구조 파괴)하여 1개 이상의 변수에 개별적 할당하는 것 말함
이터러블 또는 객체 리터럴에서 필요한 값만 추출하여 변수에 할당할 때 유용

**배열 디스트럭처링 할당 대상은 이터러블이어야 하고, 할당 기준은 배열의 인덱스 이다.**
...
배열 디스트럭처링 할당 위한 변수에 기본값 설정할 수 있다.
const [a, b, c = 3] = [1, 2]
a, b, c // 1 2 3
...
객체의 각 프로퍼티를 변수에 할당하려면 프로퍼티 키를 사용해야 한다.
...
firstName, lastName // Ungmo Lee

순서 상관없이 프로퍼티 키 기준으로 할당된다.
const {lastName, firstName} = user;
firstName, lastName // Ungmo Lee

객체 리터럴에서 선언한 변수는 프로퍼티 축약 표현 통해 선언한 것이다.
...
clear 메서드는 모든 요소를 삭제한다.
const set = new Set([1, 2, 3])
set.clear()
set // {}

set 객체는 이터러블이다. => for ... of 문, 스프레드 문법, 배열 디스트럭처링의 대상이 될 수 있다.
const set = new Set([1, 2, 3])
...
Map 생성자 함수는 키와 값의 쌍으로 이뤄진 이터러블을 인수로 받아 Map 객체를 생성한다

```js
const map1 = new Map([['key', 'value1'], ['key2', 'value2']])
mqp1 // {"key1" => "value1", "key2" => "value2"}

Map 생성자 함수에 인수로 전달한 이터러블에 중복된 키 갖는 요소 존재하면 값이 덮어써진다.
const map1 = new Map([['key', 'value1'], ['key1', 'value2']])
map1 // {"key1" => "value2"}
map1.size // 2

Map 객체에 요소 추가
set 메서드는 새로운 요소 추가된 Map 객체 반환한다.
map.set('key3', 'value3')

Map 객체는 키 타입에 제한이 없음 => 객체 포함한 모든 값을 키로 사용할 수 있다.
const map = new Map();
const lee = {name: "Lee"}
map.set(lee, 'developer')
map // {{name: "Lee"} => "developer"}
...
요소 삭제
map.delete(lee)
delete 메서드는 삭제 성공 여부 나타내는 불리언 값 반환한다.

요소 일괄 삭제
map.clear();

Map 객체는 이터러블이므로 for ... of 문, 스프레드 문법, 배열 디스트럭처링 할당 대상이 될 수 있다.
[...map]
const [a, b] = map
```
...
이벤트 핸들러 등록: 이벤트 발생했을 때 브라우저에게 이벤트 핸들러 호출 위임하는 것
...
- addEventListener('이벤트 타입', 이벤트 핸들러, capture 사용 여부)
- capture를 false 지정하면 버블링 단계에서 이벤트 캐치하고, true는 캡처링 단계에서 이벤트 캐치한다.
- 동일한 요소에서 발생한 동일 이벤트에 대해 하나 이상의 이벤트 핸들러 등록할 수 있다.
- 참조가 동일한 이벤트 핸들러 중복 등록하면 하나의 이벤트 핸들러만 등록된다.

#### 이벤트 핸들러 제거

- removeEventListener 메서드 사용
- addEventListener 에 전달한 인수와 똑같이 전달해야 이벤트 핸들러가 제거가 된다.
- 이벤트 핸들러 프로퍼티 방식으로 등록한 이벤트 핸들러는 메서드로 제거할 수 없다.(프로퍼티에 null 할당하여 제거)
...
```js
이벤트 핸들러 어트리뷰트 방식의 경우 event가 아닌 다른 이름은 이벤트 객체 전달받지 못한다.
<body onclikc="showCoords(event)">
  <em class="message"></em>
  <script>
    const msg = document.querySelector('.message');

    function showCoords(e) {
      msg.textContent = `clikcX: ${e.clientX}, clientY: ${e.clientY}`;
    }
  </script>
</body>
```
...
currentTarget 프로퍼티는 이벤트 핸들러가 바인딩된 DOM 요소 가리킨다.
DOM 요소와 이벤트 핸들러 바인딩된 요소는 모두 checkbox 이다.
```

#### 이벤트 전파

DOM 트리 상에서 DOM 요소 노드에 발생한 이벤트는 DOM 트리 통해 전파된다.

- 캡처링 단계: 이벤트가 상위 요소에서 하위 요소 방향으로 전파
- 타깃 단계: 이벤트가 이벤트 타깃에 도달
- 버블링 단계: 이벤트가 하위요소 에서 상위 요소 방향으로 전파
  ...
  <body>
    ...
    <script>
      ...
      fruits.addEventListener("click", (e) => {
        ...
        console.log(e.currentTarget); // [object HTMLUListElement]
      });
    </script>
  </body>
</html>
...
1. li 요소 클릭하면 클릭 이벤트 객체가 window에서 시작해서 이벤트 타깃 방향으로 전파된다. => 캡처링 단계
2. 클릭 이벤트 객체 -> 이벤트 타깃에 도달 => 타깃 단계
...
**이처럼 이벤트는 이벤트 발생시킨 이벤트 타깃은 물론 상위 DOM 요소에서도 캐치할 수 있다.**
  ...
  <body>
    ...
    <script>
      ...
      document.querySelector(".btn2").onclick = (e) => {
        e.stopPropagation(); => 없을 시 이벤트 위임 받아 빨간색으로 된다.
...
increase 메서드 내부의 this 는 클래스가 생성할 인스턴스를 가리키는지 않고
이벤트를 바인딩한 DOM 요소를 가리킨다. 그래서 increase 내부의 this.button 은
button.button 과 같다 그래서 increase 메서드 지정할 때 bind 메서드를 써줘야한다.
this.button.onclick = this.increase.bind(this); 아니면 increase 메서드를 화살표
함수로 만들던지 해야한다.
```
...
- 이벤트 핸들러 프로퍼티, addEventListener 메서드는 이벤트 핸들러를 브라우저가 호출하므로 함수 호출문이 아닌 함수 자체를 등록해야 해서 인수 전달할 수 없다.
...
- addEventListener 메서드로 생긴 이벤트 객체: MouseEvent {isTrusted: false, screenX: 0, screenY: 0, clientX: 0, clientY: 0, …}
- 커스텀 클릭 이벤트 생긴 객체:PointerEvent {isTrusted: true, pointerId: 1, width: 1, height: 1, pressure: 0, …}

# 41장 타이머

자바크립트 엔진은 하나의 실행 컨텍스트 스택 갖기 때문에 두 가지 이상의 태스크를 동시에 실행할 수 없다.
...
세 번쨰 인자로 인수 전달할 수 있다.
setTimeout(name => console.log(`Hi ${name}`), 1000, 'Lee')

setTimeout 함수는 생성된 타이머 식별할 수 있는 고유 타이머 id 반환한다.
...
=> 짧은 시간 간격으로 발생하는 이벤틀르 그룹화 해서 이벤트 핸들러의 호출 방지하는 프로그래밍 기법
...
> 함수가 호출된 순으로 순차적 실행되는 이유는 순서대로 함수 실행 컨텍스트가 실행 컨텍스트 스택에 푸시되기 때문
> **자바스크립트 엔진은 단 하나의 실행 컨텍스트 스택 갖는다.** => 함수 실행할 창구가 단 하나이다.
> 자바스크립트 엔진은 한번에 하나의 태스크만 실행할 수 있는 **싱글 스레드** 방식으로 동작한다.

현재 실행 중인 태스크가 종료할 때까지 다음 실행될 태스크가 대기하는 방식을 **동기 처리** 라고 한다.
...
setTimeout 함수 이후의 태스크를 블로킹하지 않고 곧바로 실행한다. 이처럼 현재 실행 중인 태스크가 종료되지 않아도 다음 태스크 곧바로
실행하는 방식을 비동기 처리라고 한다.
```
...
HTTP 요청 통해 서버로부터 데이터 가지고 오면서 렌더링하기도 하는 것 등
=> 자바스크립트의 동시성을 지원하는 것이 바로 **이벤트 루프**(브라우저에 내장되어 있는 기능 중 하나)

##### 비동기 처리에서 소스코드 평가와 실행 제외한 모든 처리는 자바스크립트 엔진 구동하는 환경인 브라우저 또는 Node.js가 담당한다.

ex) setTimeout의 콜백 함수의 평가와 실행은 자바스크립트 엔진이 담당하지만 호출 스케줄링 위한 타이머 설정, 콜백 함수 등록은
브라우저 또는 Node.js 가 담당한다.

- 태스크 큐: 비동기 함수의 콜백 함수 또는 이벤트 핸들러가 일시적으로 보관되는 영역
- 이벤트 루프: 콜 스택에 현재 실행중인 실행 컨텍스트 있는지, 태스크 큐에 대기중인 함수(콜백, 이벤트 핸들러 등)가 있는지 반복 확인한다
  콜 스택 비어있고 태스크 큐에 대기중인 함수 있으면 이벤트 루프틑 순차적으로(FIFO) 대기중인 함수 콜스택으로 이동시킨다.
...
1. 전역 코드 평가되어 전역 실행 컨텍스트 생성되고 콜 스택에 푸시
2. 전역 코드 실행되어 setTimeout 함수 호출 => setTimeout 함수의 함수 실행 컨텍스트 생성되고 콜 스택에 푸시
3. setTimeout 함수 실행되면 콜백 함수 호출 스케줄링하고 종료되어 콜 스택에서 팝된다.
타이머 설정과 타이머 만료되면 콜백 함수를 태스크 큐에 푸시하는 것은 브라우저의 역할이다.
4. 브라우저가 수행하는 4-1과 자바스크립트 엔진이 수행하는 4-2는 병행 처리된다.
  4-1. 브라우저는 타이머 설정하고 타이머 만료 기다린다. 이후 만료되면 콜백 함수 foo가 태스크 큐에 푸시된다.
    4ms 후에 만료되면 함수 foo가 태스크 큐에 푸시되어 대기하게 된다.
  4-2. bar 함수 호출되어 bar 함수 실행 컨텍스트 생성되고 콜 스택에 푸시되어 실행된다. 그후 종료되어 팝 된다.
5. 전역 코드 실행 종료되고 전역 실행 컨텍스트가 콜 스택에서 팝 된다. => 콜 스택 비게 된다.
6. 이벤트 루프에 의해 콜 스택이 비어있음을 감지하고 태스크 큐에서 대기중인 콜백 함수 foo가 이벤트 루프에 의해 콜 스택에 푸시된다.
```
...
1. 변경할 부분을 갱신하는 데 필요한 데이터만 서버로부터 전송받기 때문에 불필요한 데이터 통신 발생하지 않는다.
2. 변경할 필요 없는 부분은 렌더링되지 않는다. => 화면이 순간적으로 깜박이는 현상이 발생하지 않음
3. 클라이언트와 서버와의 통신이 비동기 방식으로 동작하기 때문에 서버에게 요청 보낸 이후 블로킹이 발생하지 않는다.

#### JSON

> JSON 은 클라이언트와 서버 간의 HTTP 통신 위한 텍스트 데이터 포맷이다.
> => 자바스크립트에 종속되지 않는 언어 독립형 데이터 포맷이므로 대부분의 프로그래밍 언어에서 사용할 수 있다.

#### JSON.stringify

> JSON.stringify 메서드는 객체를 JSON 포맷의 문자열로 변환한다.
> 클라이언트가 서버로 객체 전송하려면 객체를 문자열화해야 하는데 => 직렬화 라고 한다.

#### JSON.parse

> JSON 포맷의 문자열을 객체로 변환한다.
> 서버로부터 클라이언트에게 전송된 JSON 데이터는 문자열 이므로 객체로서 사용하려면 JSON 포맷을 객체화해야 한다. => 역직렬화 라고 한다.

#### HTTP 요청 전송

1. SMLHttpRequest.prototype.open 메서드로 HTTP 요청 초기화
...
비동기 함수 호출 시 함수 내부의 비동기 코드가 완료되지 않았다 해도 기다리지 않고 즉시 종료된다.
=> 비동기 코드에서 처리 결과를 외부로 반환하거나 상위 스코프의 변수에 할당하면 기대한 대로 동작하지 않는다.
...
비동기 함수인 setTimeout이 종료되고 바로 console.log(g)가 실행되므로 g = 0 이 반환된다.
```
...
비동기 함수의 콜백함수에서의 에러는 catch 블록에서 캐치되지 않는다.
=> 콜백 함수를 호출한 것은 setTimeout 함수가 아니기 때문이다.
...
1. 첫 번째 콜백 함수는 프로미스가 fulfilled 상태가 되면 호출된다. => 콜백 함수는 프로미스의 비동기 처리 결과를 인수로 전달 받는다.
2. 두 번째 콜백 함수는 프로미스가 rejected 상태가 되면 호출된다. => 콜백 함수는 프로미스의 에러를 인수로 전달받는다.
...
Promise.all 메서드는 인수로 전달받은 배열의 프로미스가 하나라도 rejected 상태 되면 나머지 프로미스가 fulfilled 상태
되는 것을 기다리지 않고 즉시 종료한다.

Promise.all 메서드는 인수로 전달받은 것이 프로미스가 아닌경우 Promise.resolve 메서드 통해 프로미스로 래핑한다.
단 인수는 이터러블이어야 한다.
```

##### Promise.race

이터러블을 인수로 받아서 가장 먼저 fulfilled 상태가 된 프로미스 처리 결과를 resolve 하는 새로운 프로미스를 반환한다.
...
마이크로태스크 큐는 태스크 큐보다 우선순위가 높다. => 이벤트 루프는 콜 스택 비면 마이크로태스크 큐에서 대기하는 함수를 가져와 실행한다.
```
...
1. 제너레이터 함수는 함수 호출자에게 함수 실행의 제어권을 양도할 수 있다.
   함수 호출자가 함수 실행을 중지하거나 재개시킬 수 있음
2. 제너레이터 함수는 함수 호출자와 함수의 상태를 주고받을 수 있다.
   일반 함수는 매개변수 통해 함수 외부에서 값 주입받고 함수 코드를 실행하여 결과값을 외부로 반환한다.
   즉, 함수 실행되는 동안 외부에서 함수 내부로 값 전달하여 함수 상태 변경할 수 없다.
   => 제너레이터 함수는 함수 호출자에게 상태 전달할 수 있고 함수 호출자로부터 상태 전달받을 수도 있다.
3. 제너레이터 함수를 호출하면 제너레이터 객체를 반환한다.
   제너레이터 함수 호출 시 함수 코드를 실행하는 것이 아니라 이터러블이면서 동시에 이터레이터인 제너레이터 객체를 반환한다.
...
애스터리스크(*)의 위치는 function 키워드와 함수 이름 사이 어디든지 상관없다.
제너레이터 함수는 화살표 함수로 정의할 수 없음
제너레이터 함수는 new로 생성자 함수로 호출할 수 없음
...
제너레이터 함수 호출 시 제너레이터 객체를 생성해 반환한다.
제너레이터 객체는 이터러블이면서 이터레이터 이다.

- next 메서드는 호출하면 제너레이터 함수의 yield 표현식까지 코드 블록을 실행하고 yield된 값을 value 프로퍼티 값으로,
  false를 done 값으로 갖는 이터레이터 리절트 객체를 반환한다.
- return 메서드를 호출하면 인수로 전달받은 값 value 프로퍼티 값, done 프로퍼티 값은 true인 이터레이터 리절트 객체 반환
...
**제너레이터 객체의 next 메서드에 인수는 제너레이터 함수의 yield 표현식을 할당받는 변수에 할당된다**
...
fetch 함수가 반환한 프로미스가 resolve한 처리 결과가 res 변수에 할당된다.
```
...
bar(1) // 약 6초 소요
비동기 처리 순서가 보장되어야 하므로 async, await 을 사용해야 한다.
```
...
stack 프로퍼티: 에러 발생시킨 콜 스택의 호출 정보 나타내는 문자열, 디버깅 목적으로 사용

#### throw 문

```js
try {
  에러 객체 생성한다고 에러가 발생하는 것은 아니다.
  new Error('something wrong');
} catch (err) {
  console.log(err)
}

에러 발생시키기 위해선 throw 문으로 에러 객체를 던져야 한다.
try {
...
에러는 호출자 방향으로 전파된다.
=> 콜 스택의 아래 방향(실행 중인 실행 컨텍스트가 푸시되기 전에 푸시된 실행 컨텍스트 방향)으로 전파된다.
...
foo 함수가 throw한 에러는 콜 스택 아래 방향으로 전파되어 전역에서 캐치된다.
```
...
이 때 콜 스택에 푸시된 콜백 함수의 실행 컨텍스트는 콜 스택의 가장 하부에 존재하게 된다.
=> 에러를 전파할 호출자가 존재하지 않는다.**

# 48장 모듈

모듈이란 애플리케이션 구성하는 개별적 요소로, 재사용 가능한 코드 조각을 말한다.
...
> 파일을 여러 개의 파일로 분리하여 script 태그로 로드해도 분리된 자바스크립트 파일들은 결국 하나의 자바스크립트 파일 내에 있는 것처럼 동작한다.
> => 자바스크립트 파일은 하나의 전역을 공유한다.

#### ES6 모듈(ESM)

```js
<script type="module" src="app.mjs"></script>
일반적 자바스크립트 파일이 아닌 ESM임을 명확히 하기위해 확장자는 mjs사용을 권장한다.
```

ESM은 파일 자체의 독자적 모듈 스코프를 제공한다. => 모듈 내에서 var 키워드로 선언한 변수는 더는 전역 변수가 아니며 window 객체의 프로퍼티도 아니다.
...
```js
import 하려면 export한 식별자 이름으로 import해야 하며 ESM의 경우 파일 확장자를 생략할 수 없다.
...
모듈이 export한 식별자 이름 변경하여 import 하는 방법
import {pi as PI, square as sq, Person as P} from './lib.mjs'

모듈에서 하나의 값만 export한다면 default 키워드를 사용할 수 있다.
export default x => x * x;
default 키워드 사용하는 경우 var, let, const 키워드는 사용할 수 없다.
default 키워드와 함께 export한 모듈은 {} 없이 임의의 이름으로 import 한다.

import square from './lib.mjs'
```

**그냥 변수에 export 한 것도 {} 감싸서 import 해야한다.**
...
Babel 사용하면 위 코드를 다음과 같이 ES5사양으로 변환할 수 있다.

[1, 2, 3].map(function (n) {
  return Math.pow(n, n);
})

Babel은 구형 브라우저에서도 동작하는 ES5 사양의 소스코드로 변환(트랜스파일링)할 수 있다.
```

#### Webpack

Webpack은 의존 관계에 있는 자바스크립트, CSS, 이미지 등의 리소스들을 하나의 파일로 번들링하는 모듈 번들러이다.
